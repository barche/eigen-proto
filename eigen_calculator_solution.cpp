...
{
  typedef typename boost::remove_reference<LeftT>::type left_unref;
  typedef typename boost::remove_reference<RightT>::type right_unref;
...
struct do_eigen_multiply : proto::callable
{
  ...
  template<typename ThisT, typename ExprT, typename LeftT, typename RightT>
...
struct do_wrap_expression : proto::transform< do_wrap_expression >
{
  ...
  {
    typedef typename boost::result_of<calculator_transform(typename impl::expr_param, typename impl::state_param, typename impl::data_param)>::type result_ref_type;
